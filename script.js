...
if (window.mapboxgl && document.getElementById("wineMap")) {
  ...
  map.on("load", () => {
    renderMapMarkers(visibleShops);
    fitMapToShops(visibleShops);
  });
}
...
function renderMapMarkers(list) {
  ...
  list.forEach((shop) => {
    ...
    marker.getElement().classList.add("shop-map-marker");
...
function selectShop(shopId, options = {}) {
  ...
  const card = document.querySelector(`[data-shop-id="${shopId}"]`);

  document.querySelectorAll(".shop-card").forEach((item) => {
  ...
  if (openPopup && mapMarkers[shopId]) {
    mapMarkers[shopId].togglePopup();
  }
}

function updateActiveMarker() {
  Object.entries(mapMarkers).forEach(([shopId, marker]) => {
    marker.getElement().classList.toggle("active", shopId === activeShopId);
  });
}
...
function renderShops(list) {
  ...
  list.forEach((shop) => {
    ...
    card.querySelector(".shop-link").addEventListener("click", (event) => {
